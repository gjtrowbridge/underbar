...
(function() {
  ...
  _.last = function(array, n) {
    if (n === 0) {
      ...
      return n === undefined ? array.slice(-1)[0] : array.slice(-1 * n);
    }
  };
  ...
  _.memoize = function(func) {
    ...
    return function(arg) {
      if (!(arg in alreadySeen)) {
        ...
        alreadySeen[arg] = result
      } else {
        var result = alreadySeen[arg];
      }
      return result;
    }
  };
  ...
  _.shuffle = function(array) {
    var a = array.slice();
    //loops through all elements of array except the last
    for (var i=0; i<a.length-1; i++) {
      
      //picks a random index from 0 to array length - i
      var pick = Math.floor(Math.random() * (a.length - i));
      
      //switches the ith value with the (random index + i)th value
      var temp = a[pick + i];
      a[pick+i] = a[i];
      a[i] = temp;
    }
